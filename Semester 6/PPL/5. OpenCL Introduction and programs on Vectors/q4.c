...
int main() {
    int n;
    const char *programSource = "__kernel void binaryToDecimal(__global int *input, __global int *output) { \
    int idx = get_global_id(0); \
    int binary = input[idx]; \
    int decimal = 0; \
    int base = 1; \
    while (binary > 0) { \
    ...
    cl_kernel kernel = clCreateKernel(program, "binaryToDecimal", &status);
    ...
    printf("Output (Binary => Decimal values):\n");
    for (int i = 0; i < n; i++) {
